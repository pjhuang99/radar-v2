...
【时空坐标校准】：当前真实时间是 ${timeStr}。
针对选题【${title}】和视角【${angle}】，请基于以下提供的【原始参考素材】提取最鲜活的新闻细节。`;
        researchPrompt += `\n【时间处理规则】：严禁编造时间跨度。素材没有标明时间的事件，一律按"当下正在发生"提取，不要写成"去年""此前""已经过去了一段时间""时隔多年"。只有素材明确写了年份或日期时才可引用该时间，且必须原样保留，不要改写或推算。`;
...
【时间处理规则】（最高优先级，违反即失败）：
1. 素材没有标明时间的事件，一律当作"当下正在发生"，直接陈述，不要给它附加时间跨度。
2. 严禁凭空编造时间距离：不得出现"去年""一年前""X年前""时隔多年""已经过去了一段时间""此前""当年""昔日"等表述，除非素材里明确写了该时间。
3. 严禁用"2026年X月，距离……"这类句式开篇。
4. 只有素材明确写了年份或日期时，才可引用该时间，且不得改动。
【深度素材】：
${research}
